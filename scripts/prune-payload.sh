#!/bin/bash
# 精简 server/node_modules：只保留 darwin-arm64 的原生预编译物。
# 保守清单：仅删除其他平台的预编译二进制，不触碰任何功能代码。
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
NM="$ROOT/server/node_modules"

[ -d "$NM" ] || { echo "server/node_modules 不存在，先运行 fetch-payload.sh"; exit 1; }
